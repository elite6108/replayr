...
import { readApiError, readApiJson } from "../utils/http";
import type {
...
export async function followUser(accessToken: string, username: string): Promise<FollowActionResponse> {
  const response = await fetch(`${publicApiUrl()}/v1/users/${encodeURIComponent(username)}/follow`, {
    method: "POST",
...
export async function acceptFollowRequest(accessToken: string, username: string): Promise<FollowActionResponse> {
  const response = await fetch(`${publicApiUrl()}/v1/follow-requests/${encodeURIComponent(username)}/accept`, {
    method: "POST",
    headers: authHeaders(accessToken),
  });
...
export async function declineFollowRequest(accessToken: string, username: string): Promise<void> {
  const response = await fetch(`${publicApiUrl()}/v1/follow-requests/${encodeURIComponent(username)}`, {
    method: "DELETE",
    headers: authHeaders(accessToken),
  });
...
export async function fetchFollowing(accessToken: string): Promise<FollowListItem[]> {
  const response = await fetch(`${publicApiUrl()}/v1/following`, { headers: authHeaders(accessToken) });
...
export async function fetchFollowers(accessToken: string): Promise<FollowListItem[]> {
  const response = await fetch(`${publicApiUrl()}/v1/followers`, { headers: authHeaders(accessToken) });
...
export async function fetchFollowRequests(accessToken: string): Promise<{
  ...
  const response = await fetch(`${publicApiUrl()}/v1/follows/requests`, { headers: authHeaders(accessToken) });
